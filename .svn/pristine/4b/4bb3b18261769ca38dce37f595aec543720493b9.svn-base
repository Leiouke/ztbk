...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
				...
				//bootstrap初始化
				initialize_table($table,"getcashClaimDetailList",${tableHeader},queryParams,"#tableTool",400);
				//表格配置 layer
				var $config=$("#config");
		...
		function renderOperator(value,row,index){
			 var str="";
			 if(row.audit_status==1 && row.audit_pass == 1
					 && row.memo != '直接发票认领') {  // 审核通过的客户认领部分才可以作废, 发票认领部分不可以作废！（这里直接写memo有隐患）
