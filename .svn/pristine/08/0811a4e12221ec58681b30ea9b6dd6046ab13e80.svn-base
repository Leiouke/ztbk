		            ...
		            <div class="ibox-content search-content">
			            <form id="form" class="form-inline">
				            <div class="col-xs-12">
								...
								<div>
									<input type="hidden" name="downshelf_id" id = "downshelf_id" class="form-control search-items" >
								</div>
							 </div>
						</form>
		            </div>
...
<!-- 	    <button id="commit" class="btn btn-primary" type="button">确认</button> -->
<!-- 	</div> -->
<!-- 	<div class="sonpage-footer"> -->
	<div class="row row-lg" style="margin-top: 5px;margin-bottom: 5px;text-align: center;">
<!-- 	    <button id="save" class="btn btn-primary" type="button" style = "margin-right: 15px">保存</button> -->
	    <button id="cancel" class="btn btn-white" type="button" style = "margin-right: 15px">取消</button>
<!-- 	    <button id="commit" class="btn btn-primary" type="button" style = "margin-right: 15px">保存并确认</button> -->
	</div>
	</body>
	<!-- Bootstrap table -->
	...
	var outBound_item = JSON.parse('${oneJson}');
	$(document).ready(function () {
		
// 		if(outBound_item.is_verify==1){
// 			$('#save').hide();
// 			$('#commit').hide();
// 			$('#delete_items').hide();
// 			$('#add').hide();
// 		}
		...
		//加载字典数据
		get_dictionary("${context}", outBound_item);
		
		//填写已有数据
		set_one_info($("#form"), outBound_item);
...
// 				ajax_function_ask_and_close("确认修改？","editOutBound","post",check,'修改成功');
// 			}
...
// 				ajax_function_ask_and_close("确认提交？","editOutBound","post",check,'修改提交成功');
// 			}
...
// 		});
		...
		//bootstrap初始化		
		initialize_table_editable($table,"getOutBoundItemTempList",${tableHeader},queryParams,"#tableTool",550,"updateOutBoundItemTemp");
