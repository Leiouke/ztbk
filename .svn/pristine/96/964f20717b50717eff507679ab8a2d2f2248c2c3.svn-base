...
<html lang="zh-CN">
	...
	<div class="sonpage-footer-left">
    	<button id="summary" class="btn btn-info" type="button">月度汇总</button>
    	<!-- <button id="summaryPurchase" class="btn btn-info" type="button">财务明细月度汇总</button> -->
	</div>
	</body>
	...
	<script type="text/javascript">
		...
		function queryParams(param) {
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $('#account_commit').on('click',function(){
				 if(!has_null($("#account_form"))){
						var fh_ids = $.map($table.bootstrapTable('getSelections'), function (row) {
			                return row.fh_id;
			            });
						var account_month=$("input[name='account_month']").val();
						if(fh_ids.length!=0){
							ajax_function_ask_and_refresh("确认记账？","accountFhSh","post",{'fh_ids':fh_ids,'account_month':account_month},'操作成功',$table);
			 ...
			 $('#return_account').on('click',function(){
				 if(true){
					...
					if(fh_ids.length!=0) {
						ajax_function_ask_and_refresh("确认撤销？","deleteFhshAccount","post",{'fh_ids':fh_ids},'操作成功',$table);
					} else {
						layer.msg("未选择明细！");
					}
				 }
			 ...
			 $("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getFhShAccountList",$table);
			 });
			 //自动搜索填补
			 ...
			 button_click_layer($("#summary"),2,"月度汇总",'accountSummary',true,['620px', '580px']);  // 月度汇总是进货报表的
			 
			 button_click_layer($("#summaryPurchase"),2,"财务明细月度汇总",'accountPurchaseSummary',true,['620px', '580px']);  // 月度汇总是进货报表的
	     });
		 
		 function toDetail(fh_id){
			 open_layer(2,'申汇单详情','fhShAccountDetail?fh_id='+fh_id,$table,true);
		 }
		 ...
		 function renderOperator(value,row,index){
			 var str="<a onclick=\"toDetail('"+row.fh_id+"')\">详情</a>";
			 if(true){
