...
<html lang="zh-CN">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">	
					  ...
					  <div class="form-group inline-small">
					    <label >订阅日期 从：</label>
					    <input type="text" class="form-control  layer-date search-items" name="period_start" id="period_start">
					    -
					            到<input type="text" class="form-control  layer-date search-items" name="period_end" id="period_end">
					  </div>
					  <div class="form-group inline-small">
					    <label >客户名称：</label>
	...
	<script type="text/javascript">	
		...
		function queryParams(param) {
		 ...
		 $(document).ready(function () {
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getOrderDeliverySearchList",$('#table'));
			 });
		 ...
		 function toDetail(qk_id, custom_id, period_start, period_end) {
			 open_layer_refresh(2,'配送明细','relateDeliveryItems?qk_id=' + qk_id + '&custom_id=' + custom_id
					 + '&period_start=' + period_start + '&period_end=' + period_end,$table,['750px', '500px'],true);
		 }
		 
		 function renderOperator(value,row,index){
			 var str="<a onclick=\"toDetail('"+row.qk_id + "','" + row.custom_id + "','" + format_date(row.period_start) + "','" + format_date(row.period_end)  +"')\">详情</a>";
			 return str;
		 }
	</script>
</html>