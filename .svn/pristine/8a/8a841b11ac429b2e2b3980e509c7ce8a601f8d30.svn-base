			   ...
			   <div class="col-md-12">
			        <div class="row">
				    	<div class="form-group large-space col-xs-12 not-null">	
	                  		<label>报表类型：</label>
	                  		<c:if test="${param.report_type == 0}">
	                  			<input type="text" name="report_type_name" value="业务销售报表"  disabled>
	                  		</c:if>
	                        <c:if test="${param.report_type == 1}">
	                  			<input type="text" name="report_type_name" value="其他收入报表"  disabled>
	                  		</c:if>
	                  		<c:if test="${param.report_type == 2}">
	                  			<input type="text" name="report_type_name" value="预收销售报表"  disabled>
	                  		</c:if>
	                  		<c:if test="${param.report_type == 3}">
	                  			<input type="text" name="report_type_name" value="预收冲销报表"  disabled>
	                  		</c:if>
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$('#save').on('click', function () {
				if(!has_null($("#form"))){
					ajax_function_ask_and_close("确认新增报表？", "addSalesReport", "post", $("#form").serialize(), "新增完成！");
				}
			 });
		});
	</script>
