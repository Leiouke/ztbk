            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group inline-middle">
					    <label >财务编码：</label>
					    <input type="text" name="f_account_id" class="form-control search-items" >
					  </div>
				 </div>
	...
	<script type="text/javascript">
		...
		function queryParams(param) {
		 ...
		 $(document).ready(function () {
			...
			//bootstrap初始化
			initialize_table_no_footer($table,"",${tableHeader},queryParams,"#tableTool");
			$("#search").on('click',function(){
				 var json_data_list=formToJson($("#form_search"));
				 json_data_list['queryId']='${queryId}';
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getFactoryList",$('#table'));
			 });
			//增加条目layer
			var $add=$("#add");
			button_click_layer_refresh($add,2,"新增货源",'factoryAdd',$table,true,['620px', '580px']);
			
			//增加条目layer
			var $download=$("#download");
			button_click_layer_refresh($download,2,"下载益华货源",'factoryDownload',$table,false,['800px', '260px']);
			...
			initialize_bsSuggest_fc_user("${context}", "charge_man_name", "charge_man");      // 放用户信息
		 ...
		 function renderOperator(value,row,index){
			 //console.log(row);
			return "<a onclick=\"toEdit('"+row.f_id+"')\">修改</a>&nbsp;&nbsp;<a onclick=\"toDel('"+row.f_id+"')\">删除</a>";
		 }
		 
		 function toDel(f_id){
			ajax_function_ask_and_refresh("是否删除","deleteFactory","post",{'f_ids':f_id},"删除成功",$('#table'));
		 }
		 ...
		 /*$('#batchImport').on('click', function () {
			 ajax_function_ask_and_refresh("确认要同步吗？","syncClientAndFactory","post",{},"同步完成",$('#table'));
		 });*/
	</script>
</html>