...
<html lang="zh-CN">
			...
			<div class="col-md-12">
			   <div class="ibox float-e-margins">
		            ...
		            <div class="ibox-content search-content">
					  ...
					  <div class="form-group inline-small">
					    <label >是否已下架：</label>
					    <select name="is_downshelves" class="form-control search-items">
	...
	<div class="sonpage-footer">
	    <button id="pull_in" class="btn btn-primary" type="button">引入</button>
	...
	$(document).ready(function () {
		...
		$('#pull_in').on('click', function () {
			var downshelf_ids = $.map($table.bootstrapTable('getSelections'), function (row) {
                return row.downshelf_id;
            });
			if(downshelf_ids.length!=0) {
				ajax_function_ask_and_close("确认引入？","addSendOutItemTemp","post",{'downshelf_ids':downshelf_ids},"引入成功");
				parent.$('#source_orderno').val(downshelf_ids[0]);
			} else {
				layer.msg("未选择明细！");
			}
		...
		$("#search").on('click',function(){
			 ...
			 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getSendOutItemPullInList",$table);
		 });
