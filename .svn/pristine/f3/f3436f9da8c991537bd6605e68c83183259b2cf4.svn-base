		 ...
		 $(document).ready(function () {
			...
			$('#commit').on('click', function () {
				...
				if(inv_ids_money.length!=0) {
					ajax_function_ask_and_close_json_data("确认引入？","pullAgentFinanceFhshItem","post",JSON.stringify(inv_ids_money),'引入成功');
				} else {
					layer.msg("未选择明细！");
				}
			});			
			...
			$("#search_pull_items").on('click', function () {
				$table_result.bootstrapTable('refreshOptions',{url: 'getFinanceAgentFhshPullList'});
			});
			...
			$("#select_in").on('click', function () {
				...
				var delete_num=0;//记录已经删除的数量
				var select_list=$('#table_result').bootstrapTable('getSelections');
				var pull_in_list=$('#table_pull_in').bootstrapTable('getData');
				$.each($('#table_result').bootstrapTable('getSelections'),function(index,result_row){
					$.each($('#table_pull_in').bootstrapTable('getData'),function(index,row){
					 	if(row.inv_id==result_row.inv_id){
					 		...
					 		select_list.splice(index-delete_num,1);
							delete_num++;
					 	}
				 	});
				});
				$table_pull_in.bootstrapTable('append',select_list);
				if(!has_all_pull_in) {
					layer.msg("有明细已经添加！");
				}
			});
