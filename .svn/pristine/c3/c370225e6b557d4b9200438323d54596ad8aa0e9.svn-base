...
<html lang="zh-CN">
        ...
        <div class="ibox float-e-margins" style="margin-bottom:0px">
            ...
            <div class="ibox-content ibox-content-table">
                <div class="row row-lg">   
                    <div class="col-sm-12"> 
                    	<div class="btn-group hidden-xs" id="tableTool" role="group">
                    	        <button id="commit" class="btn btn-outline btn-primary" type="button">提交</button>
                ...
                <div class="row row-lg" style="margin-top: 5px;text-align: center;">
               		<button type="button" class="btn btn-success" id="commit1">提交</button>
                </div>
            </div>
        </div>
    </div>
	...
	<script type="text/javascript">
		...
		function commit() {
			var rs_ids_money= $.map($table.bootstrapTable('getSelections'), function (row) {
                return {'rs_id':row.rs_id,'against_real_money':row.no_against_money};
            });
			if(rs_ids_money.length!=0) {
				ajax_function_ask_and_close_json_data("确认引入？","pullStatementItem","post",JSON.stringify(rs_ids_money),'引入成功');
			} else {
				layer.msg("未选择明细！");
			}
		}
		...
		$(document).ready(function () {
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getStatementItemPullList", $table);
			});
