	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$('#commit').on('click', function () {
				var inv_ids= $.map($table_result.bootstrapTable('getSelections'), function (row) {
	                return {'invoice_id':row.inv_id};
	            });
				if(inv_ids.length!=0) {
					ajax_function_ask_and_close_json_data("确认引入？","pullDhBatchItem","post",JSON.stringify(inv_ids),'引入成功');
				} else {
					layer.msg("未选择明细！");
				}
			});
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getDhBatchPullList", $table_result);
			});
