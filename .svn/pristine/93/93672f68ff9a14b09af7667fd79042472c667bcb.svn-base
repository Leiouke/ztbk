...
<!-- 				 </div> -->
	...
	<script type="text/javascript">		
		...
		$(document).ready(function () {
			...
			initialize_table($table,"getSingleMenuList",${tableHeader},queryParams,"#tableTool",500);
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getSingleMenuList",$('#table'));
			});
...
// 			get_assess_unit_by_dept("${context}", "unit_code");
		 ...
		 function toMenu(account){
			 open_layer_refresh(2,'编辑菜单','singleMenuEdit?account='+account,$table,['850px', '450px'],false);
		 }
		 
		 function renderOperator(value,row,index) {
			var str="";
			str += "<a onclick=\"toMenu('"+row.account+"')\">菜单</a> ";
			return str;
		 }
	</script>
</html>