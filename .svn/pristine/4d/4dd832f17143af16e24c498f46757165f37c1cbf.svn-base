	...
	<script type="text/javascript">
		...
		function commit() {
			var so_ids_money= $.map($table.bootstrapTable('getSelections'), function (row) {
                return {'statement_id':row.rs_id, 'kp_money':row.no_kp_money};
            });
		...
		$(document).ready(function () {
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getSaleInvoiceStatementItemPullList", $table);
			});
