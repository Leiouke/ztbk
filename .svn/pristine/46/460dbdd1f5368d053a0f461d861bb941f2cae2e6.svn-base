			   ...
			   <div class="col-md-12">
			   	    <div class="row">
				    	<div class="form-group small-space col-xs-4">	
	                  		<label>期刊刊号：</label>
	                        <input type="text" name="qk_no" readonly>
		                </div>
		                <div class="form-group small-space col-xs-4">	
	                  		<label>期刊名称：</label>
	                        <input type="text" name="qk_name" readonly>
		                </div>
				    </div>
				    <div class="row">
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$('#save').on('click', function () {
				if(!has_null($("#form"))){
					ajax_function_ask_and_close("确认修改财务分类？","editQkNoCwtypeMapping","post",$("#form").serialize(),"修改完成！");
				}
			 });
		});
	</script>
