...
<html lang="zh-CN">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					 ...
					 <div class="form-group input-group inline-middle">
					  ...
					  <div class="form-group inline-small">
					    <label >是否认领完：</label>
					    <select name="receive_out" class="form-control dictionary isboolean">
					    </select>
					  </div>
					  <div class="form-group inline-small">
					    <label >是否认领过：</label>
	...
	<script type="text/javascript">
		...
		function queryParams(param) {
		 ...
		 $(document).ready(function () {
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $("#search").on('click',function(){
				...
				search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getCashClaimList",$table);
			});
			 //自动搜索填补
		 ...
		 function toClaim(rp_id){
			 open_layer_refresh(2,'认领','cashClaimAdd?rp_id='+rp_id,$table,['720px', '580px'],false);
		 }
		 
		 function toDetail(rp_id){
			 open_layer_refresh(2,'认领详情','cashClaimDetail?rp_id='+rp_id,$table,['620px', '580px'],true);
		 }
		 
		 function toClaimInvoice(rp_id){
			 open_layer_refresh(2,'认领','cashClaimInvoiceManage?rp_id='+rp_id,$table,['720px', '580px'],true);
		 }
		 
		 function renderOperator(value,row,index){
		 	var str="";
		 	if(row.receive_out==0) {
		 		str+="<a onclick=\"toClaim('"+row.rp_id+"')\">客户认领</a>&nbsp;&nbsp;";
		 	}
		 	if(true) {
		 		str+="<a onclick=\"toClaimInvoice('"+row.rp_id+"')\">发票认领</a>&nbsp;&nbsp;";
		 		str+="<a onclick=\"toDetail('"+row.rp_id+"')\">详情</a>";
		 	}
			return str;
		 }		 
	</script>
