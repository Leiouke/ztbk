...
<html lang="zh-CN">
	...
	<script type="text/javascript">		
		...
		$(document).ready(function () {			
			...
			initialize_table_singleSelect($table,"",${tableHeader},queryParams,"#tableTool",600);
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getSectionBdPullList",$('#table'));
			});
		...
		$("#batchImport").on('click',function() {
			...
			var item_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
                return {'bd_id':row.bd_id};
            });
			
			if(item_ids.length != 0) {
				ajax_function_ask_and_close_json_data("确认引入?","importSectionBd","post",JSON.stringify(item_ids),"引入完成!");
			} else {
				layer.msg("未选择明细！");
			}
		});
