...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			//bootstrap初始化
			initialize_table($table_result, "getIncomingClaimItemItemPullList", ${tableHeader}, queryParams_result, "#tableTool_result", 350);
			$("#select_in").on('click', function () {			
				...
				if(item_ids.length != 0) {
					ajax_function_ask_and_close_json_data("确认提交?","pullIncomingClaimItemItem","post",JSON.stringify(item_ids),"引入完成!");
				} else {
					layer.msg("没有选中发票记录！");
				}
			});
			...
			$("#search").on('click',function(){
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getIncomingClaimItemItemPullList", $table_result);
			});
	     });
	</script>
</html>