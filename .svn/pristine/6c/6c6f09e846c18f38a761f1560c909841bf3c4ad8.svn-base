	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getAdvancePaymentManageList",$table);
			 });
			//增加条目layer
			var $add=$("#add");
			button_click_layer_refresh($add,2,"新增代销预付单",'advancePaymentAdd',$table,true,['620px', '500px']);
		 ...
		 function toEdit(ap_id){
			 open_layer_refresh(2,'编辑费用支付单','advancePaymentEdit?ap_id='+ap_id,$table,['620px', '580px'],true);
		 }
		 ...
		 function renderOperator(value,row,index){
			...
			if (row.is_confirm == '0') {
		    	...
		    	//str+="&nbsp;&nbsp;<a onclick=\"toCommit('"+row.pc_id+"')\">作废</a>";
		    }
			return str;
		 }
	</script>
