...
<html lang="zh-CN">
        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group input-group inline-small">
						    <label >指派一审人员：</label>
						    <input type="text" name="zpzs_man_name" class="form-control search-items" value="${sessionScope.user.userName}">
	                        ...
	                        <input type="hidden" name="zpzs_man" class="form-control search-items" value="${sessionScope.user.job_no}">
					  </div>
					  <div class="form-group inline-middle">
				  	  ...
				  	  <div class="form-group inline-small">
					    <label >是否已指派：</label>
					    <select name="is_zp" class="form-control dictionary search-items isboolean" id = "is_zp" >
					    	<option value=""></option>
					    </select>
					  </div>
					  <div class="form-group inline-small">
					    <label >配送日期：</label>
					    <input type="text" class="form-control  layer-date search-items" name="ps_date_start" id="ps_date_start">
					    -
					    <input type="text" class="form-control  layer-date search-items" name="ps_date_end" id="ps_date_end">
					  </div>
					  <div class="form-group inline-small">
					    <label >希望审读日期：</label>
					    <input type="text" class="form-control  layer-date search-items" name="hope_date_start" id="hope_date_start">
					    -
					    <input type="text" class="form-control  layer-date search-items" name="hope_date_end" id="hope_date_end">
					  </div>
				 </div>
    ...
    <div class="sonpage-footer">
		...
		<button id="batchZpzs" class="btn btn-warning" type="button">批量审读派发</button>
	</div>
	</body>
	...
	<script type="text/javascript">			
		...
		$(document).ready(function () {			
			...
			initialize_table($table,"getProductItemsList",${tableHeader},queryParams,"#tableTool",600,rowStyle);
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getProductItemsList",$('#table'));
			});
	    });
		...
		function renderOperator(value,row,index) {
			...
			if(!(row.zs1_result == 1||row.zs1_result ==2)){
				str += "<a onclick=\"toZsAssign('"+row.ItemExclusiveCode+"')\">审读派发</a> ";
			}
			return str;
		}
		
		function toZsAssign(ItemExclusiveCode) {
			open_layer_refresh(2,'审读派发','politicalZsAssignManage?ItemExclusiveCode=' + ItemExclusiveCode,$table,['400px', '400px'],false);
		}
		...
		//批量审读派发
		$("#batchZpzs").on('click',function() {
			var item_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
                return {'ItemExclusiveCode':row.ItemExclusiveCode};
            });
			
			if(item_ids.length == 0) {
				layer.msg("未选择明细！");
			}else{
				var index= open_layer(2,'审读派发','politicalZsAssignBatchManage',['400px', '400px'],false);
			}
		});
		
		//批量审读派发回调函数
		function callback(zpzs_man,hope_date){
			var item_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
                return row.ItemExclusiveCode;
            });
		    
			ajax_function_ask_and_refresh("确认指派？","politicalZsBatchAssign","post",{'zpzs_man':zpzs_man, 'hope_date':hope_date, 'item_ids':item_ids},'指派成功',$table);
