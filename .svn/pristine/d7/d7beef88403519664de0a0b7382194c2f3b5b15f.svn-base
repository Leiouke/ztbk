	...
	<script type="text/javascript">
		...
		function queryParams(param) {
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $("#search").on('click',function(){
				 if(!has_null($("#form_search"))&&!auto_not_matching($("#form_search"))){
					...
					search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getClientSurplusMoneyManageList",$table);
				 }
			 });
			 //自动搜索填补
		 ...
		 function toDetail(c_id,pre_id){
			 open_layer(2,'客户预收管理','clientSurplusMoneyDetail?c_id='+c_id+'&pre_id='+pre_id,['620px', '580px'],true);
		 }
