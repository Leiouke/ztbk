...
// 	}
	...
	$(document).ready(function () {
		...
		//增加条目layer
// 		var $add=$("#add");
// 		button_click_layer_refresh($add,2,"新增报订明细",'qkBdItemPeriodTempAdd',$table,false,['800px', '300px']);
		
// 		$('#save').on('click', function () {
			
// 			if(!has_null($("#form"))){
// 				ajax_function("editQkPeriodBd","post",$('#form').serialize(),'保存成功');
// 			}
...
//             });
// 			if(inc_nos.length!=0) {
// 				//ajax_function_ask_and_dosuccess("确认删除？","deleteQkBdItemPeriodList","post",{'inc_nos':inc_nos},load_table_data_success);
// 				ajax_function_ask_and_refresh("确认删除?","deleteQkBdItemPeriodList","post",{'inc_nos':inc_nos},"删除成功!",$('#table'));
// 			} else {
// 				layer.msg("未选择明细！");
// 			}
// 		});
		...
		//bootstrap初始化		
		initialize_table($table,"getPeriodDetailList",${tableHeader},queryParams,"#tableTool",550);
...
// 					sum_real_money+=0;
// 				}else{
// 					sum_real_money+=row.real_price;
// 				}
// 			});
//  			sum_money=sum_money.toFixed(2);
//  			sum_real_money=sum_real_money.toFixed(2);
//  			$("input[name='total_money']").val(sum_money);
//  			$("input[name='total_count']").val(sum_count);
//  			$("input[name='total_amount']").val(sum_amount);
//  			$("input[name='total_real_money']").val(sum_real_money);
// 		}});
		
		//表格配置 layer
		var $config=$("#config");
		
		var qk_bk = JSON.parse('${oneJson}');
		
		//加载字典数据
		get_dictionary("${context}", qk_bk);
		
		//填写已有数据
		set_one_info($("#form"), qk_bk);
	...
	function toDel(inc_no){
		//ajax_function_ask_and_dosuccess("确认删除？","deleteQkBdItemPeriodList","post",{'inc_nos':inc_no},load_table_data_success);
		ajax_function_ask_and_refresh("确认删除?","deleteQkBdItemPeriodList","post",{'inc_nos':inc_no},"删除成功!",$('#table'));
	}
	
	 //明细页面
// 	 function toDetail(qk_id,period_id){
// 		 open_layer_refresh(2,'报订数据关联订单明细','qkBdZdSubDetail?qk_id='+qk_id+'&period_id='+period_id,$table,['620px', '580px'],true);
// 	 }	
	...
	function renderOperator(value,row,index){
	   ...
	   str += "&nbsp;&nbsp;";
// 	   str += "<a onclick=\"toDetail('" + row.qk_id + "','" + row.period_id + "')\">明细</a>";
// 	   str += "&nbsp;&nbsp;";
