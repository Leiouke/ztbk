...
<html lang="zh-CN">
	...
	<script type="text/javascript">		
		...
		function end_function(index, layero){
			ajax_function_no_close("returnAdvance","post",{},null);
		...
		$(document).ready(function () {
			...
			initialize_table($table,"getAdvanceList",${tableHeader},queryParams,"#tableTool",500);
			
			//增加条目layer
			var $add=$("#add");
			button_click_windows_end($add,2,"新增预收单",'advanceAdd',true,['620px', '500px'],end_function);
		 ...
		 function toDel(ra_id) {
			ajax_function_ask_and_refresh("确认删除?","deleteAdvance","post",{'ra_id': ra_id},"删除成功!",$('#table'));
		 }
		 
		 function toEdit(ra_id){
			 open_layer_end(2,'编辑预收单','advanceEdit?ra_id='+ra_id,['620px', '580px'],true,end_function);
		 }
		 
		 function renderOperator(value,row,index) {
			var str="";
			str += "<a onclick=\"toEdit('"+row.ra_id+"')\">冲销</a> ";
			str += "<a onclick=\"toDel('"+row.ra_id + "')\">删除</a> ";
			return str;
		 }
	</script>
</html>