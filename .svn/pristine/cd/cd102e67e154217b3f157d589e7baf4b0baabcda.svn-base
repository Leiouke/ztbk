...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			var $redadd=$("#red_add");
			button_click_windows_end($redadd,2,"新增红票",'redInvoiceAdd',true,['620px', '500px'],red_end_function);
		 ...
		 function toEditRed(id){
			 open_layer_end(2,'编辑红票','redInvoiceEdit?inv_id='+id,['620px', '580px'],true,red_end_function);
		 }
		 ...
		 function renderOperator(value,row,index){
			...
			if(row.is_red==1){
				if(row.status==0){
					...
					//str+="<a onclick=\"toPurchase('"+row.inv_id+"')\">财务进货</a>";
				}else{
