...
public class ClientController extends ZtbkServiceController {
	private int clientSearchQueryId = 5000001;
	
	private int clientCLevelSearchQueryId = 5000009;
	
	private int clientOneQueryId = 5000001;
	
	private int clientQueryId = 5000004;	
	private GridHeadConfig clientGridHeadConfig;
	
	private GridHeadConfig bankGridHeadConfig;	
	private int bankQueryId = 5000005;
	...
	@ResponseBody
	public Object getClientListSearch(String keyword,String attribute){
		Map<String,Object> postData=new HashMap<>();
		postData.put("keyword", keyword);
		if(attribute!=null){
			postData.put("attribute", attribute);
		}
    ...
    public ModelAndView clientManage(HttpSession httpSession) throws JsonProcessingException {
        ...
        mv.setViewName("np/basic/clientManage");
    ...
    public ModelAndView clientAdd(HttpSession httpSession) throws JsonProcessingException{
        ModelAndView mv = new ModelAndView();
        mv.setViewName("np/basic/clientAdd");
        return mv;
    }
    ...
    public Object addClient(@RequestParam Map postData,HttpSession httpSession){
		// 入参, 注意按照顺序
		String paramNames[] = {"finance","c_department","c_level","o_id","cm_id","c_mem","c_charge_man",
				"attribute","a_id","p_id","city_id","c_hy_id","c_department_simple","c_money","c_law_man",
				"limit_money","limit_day","c_discount","c_back","back_limit_date","parent_client","custom_id",
				"c_ms_id","c_department_Foreign","decimal_place",
				"profit_rate"};
		// 出参, 有顺序
		String returnNames[] = {"c_id"};
		// 加入sp的名称
		postData.put("spName", "u_basic_clients_new");
...
//		System.out.println(postData.get("h_id"));
    ...
    public ModelAndView clientEdit(@RequestParam Map postData,HttpSession httpSession) throws JsonProcessingException{  // 注意这个postData里面已经包含了id的字段值
        ...
        mv.setViewName("np/basic/clientEdit");
    ...
    public Object editclient(@RequestParam Map postData){
		// 入参
		String paramNames[] = {"c_id","finance","c_department","c_level","cm_id",
				"c_mem","c_charge_man","attribute","a_id","p_id","city_id","c_hy_id",
				"c_department_simple","c_money","c_law_man","limit_money","limit_day",
				"c_discount","c_back","back_limit_date","parent_client","custom_id",
				"c_ms_id","c_department_Foreign","decimal_place","profit_rate"};
		// 加入sp的名称
		postData.put("spName", "u_basic_clients_update");
    ...
    public Object deleteClient(@RequestParam Map postData,HttpSession httpSession,HttpServletRequest request){
		...
		if(c_ids!=null&&c_ids.length>0){
			for(int i=0;i<c_ids.length;i++){
				...
				// 加入sp的名称
				postData.put("spName", "u_basic_clients_delete");				
