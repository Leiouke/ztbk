...
<html lang="zh-CN">
				           ...
				           <div class="row">
							...
							<div class="row">
								  ...
								  <div class="form-group inline-small not-null">
			                        ...
			                        <input type="hidden" name="ps_type"/>
					              ...
					              <div class="form-group inline-small not-null">	
				                  		<label>成本分类：</label>
				                        <select name="cost_classification" class="dictionary">
								    	</select>
					              </div>
					              <div class="form-group inline-small not-null">	
				                  		<label>成本类型：</label>
				                        <select name="cost_form" class="dictionary">
								    	</select>
					              </div>
					              </c:if>
							</div>
	...
	<script type="text/javascript">
		...
		function toEdit(inc_no) {
			open_layer_end(2,'编辑财务分类','statementCwItemEdit?ps_type=${param.ps_type}&inc_no='+inc_no,['620px', '500px'],false,end_function);
		}
		...
		function end_function(index, layero){
			load_sum_list("${context}/csu/getTableCollectData",{'queryId':'${queryId}','guidname':'payment_statement_guid'},$table);
     	}
		
		load_sum_list_first("${context}/csu/getTableCollectData",{'queryId':'${queryId}','guidname':'payment_statement_guid'});
		$(document).ready(function () {
			...
			var statement = JSON.parse(f_str);
			...
			$("input[name='f_name']").val(statement['f_account']+"_"+statement['f_department']);
			
			// 假如 已记账 或者 已经有开票 了 或者 已经有被冲销了  那么 不允许编辑了
			if (statement.is_account == 1 || statement.inv_money != 0 || statement.inv_real_money != 0) {
				$("#save").hide();
				$(".search-items").each(function(){
					$(this).attr("readonly",true);
				});
			} // 假如是引入的结算单, 只能修改明细, 表头不修改
			...
			initialize_table_editable($table, "getStatmentAddItemList", ${tableHeader}, queryParams, "#tableTool", 550, "editStatementItemBackrate?ps_id=" + statement.ps_id)
			...
			button_click_windows_end($add,2,"新增分类明细",'statementCwItemAdd?ps_type=${param.ps_type}',false,['620px', '500px'],end_function);
		...
		function toEditRate(inc_no, ps_cw_type) {
			open_layer_end(2,'编辑退税税率','statementItemBackrateEdit?ps_id=${param.ps_id}&inc_no=' + inc_no + '&ps_cw_type=' + ps_cw_type,['420px', '300px'],false,end_function);
		}
		
		function renderRate(value,row,index) {
			var str = "";
			str += " <a onclick=\"toEditRate('" + row.inc_no + "','" + row.ps_cw_type + "')\">" + value + "</a>";
			return str;
		}
	</script>
</html>