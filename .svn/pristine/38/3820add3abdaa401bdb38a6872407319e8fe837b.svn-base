...
<html lang="zh-CN">
	...
	$(document).ready(function () {
		$('#save').on('click', function () {
			if(!has_null($("#form"))&&!auto_not_matching($("#form"))){
				if($("input[name='f_id']").attr("data-id")==""){
					...
					ajax_function("editAdvancePayment","post",$('#form').serialize(),'保存成功');								
				}
			}
		});
		...
		$('#delete').on('click',function(){
			 ajax_function_ask_and_close("确认删除？","deleteAdvancePayment","post",{ap_id:finance_fhsh.ap_id},'删除成功');
		});
	});
