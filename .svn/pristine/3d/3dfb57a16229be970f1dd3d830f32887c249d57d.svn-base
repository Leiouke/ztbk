	...
	<script type="text/javascript">
		...
		function queryParams(param) {
			...
			json_obj['limit'] =param.limit;
   	     	...
   	     	json_obj['sortName'] =this.sortName;
   	  		...
   	  		json_obj['order_no'] = '${param.order_no}';
   	        return json_obj;
		}
		...
		$(document).ready(function () {
			...
			if('${param.detail}'==1){
			 ...
			 //固定列
			//bootstrap初始化
			initialize_table($table,"getPaymentInvoicePic",${tableHeader},queryParams,"#tableTool",400);
		...
		$('#upload').on('click', function () {
			 $.ajaxFileUpload({ 
					url:'addPaymentInvoicePic?order_no=${param.order_no}',			  
			        secureuri:false,
			        method:'post',
		...
		function toDel(pic_id){
			ajax_function_ask_and_refresh("确认删除？","deletePaymentInvoicePic","post",{'pic_id':pic_id},"删除成功！", $table);
		}
		
		function toOpenPic(pic_name){
			var tempwindow=window.open('_blank');
			tempwindow.location = '../../../images/payment_invoice/' + pic_name;
		}
...
// 			str += "<a href='file:///"+row.pic_path+"'>查看</a>"
