...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			if (${param.rs_type == 0}) {
				// 获得当前部门的财务分类(取普通型)
				get_financial_classify_by_dept("${context}", "${sessionScope.defaultRole.dep_org_code}", false, "rs_cw_type", invoice.rs_cw_type);
			} else if (${param.rs_type == 1}) {
				// 获得当前部门的财务分类(取其他型)
				get_financial_classify_by_dept("${context}", "${sessionScope.defaultRole.dep_org_code}", true, "rs_cw_type", invoice.rs_cw_type);
			}
