	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$('#commit').on('click', function () {
				var kp_id= $.map($table_result.bootstrapTable('getSelections'), function (row) {
					return {'kp_id':row.kp_id,'kp_mail':row.kp_mail,'invoice_type':row.invoice_type,'tax_rate':row.tax_rate,'tax_money':row.tax_money,'kp_no':row.kp_no};
	            });
				if(kp_id.length==0){
					layer.msg("未选择明细！");
				}else if (kp_id.length>1){
					layer.msg("只能选择一条明细对冲，请重新选择");
				}else{
					parent.$("input[name='hedging_kp_id']").val(kp_id[0].kp_id);
					parent.$("input[name='hedging_kp_no']").val(kp_id[0].kp_no);
					parent.$("input[name='kp_mail']").val(kp_id[0].kp_mail);
					parent.$("input[name='tax_money']").val(-kp_id[0].tax_money);
					parent.$("select[name='invoice_type']").val(kp_id[0].invoice_type);
					parent.$("select[name='tax_rate']").val(kp_id[0].tax_rate);
					ajax_function_ask_and_close_json_data("确认引入？","pullSaleInvoiceRedItem","post",JSON.stringify(kp_id),'引入成功');
				}
			});
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getSaleInvoiceRedPullList", $table_result);
			});
			
			//自动搜索填补
			initialize_bsSuggest_client_by_input("${context}", "c_name", "c_id");
	     });
	</script>
</html>