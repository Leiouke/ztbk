...
public class ExcelUtil1 {
	private static List<CellStyle>  columnStyleList = new ArrayList<CellStyle>();
	
	// 一次循环处理后不要忘记清空这个
	public static void emptyColumnStyleList(){
		columnStyleList = new ArrayList<CellStyle>();
	}
	/**
     * 行复制功能
     *
    ...
    public static void copyRow(Workbook wb, Row fromRow, Row toRow, boolean copyValueFlag) {
        ...
        for(Iterator cellIt = fromRow.cellIterator(); cellIt.hasNext(); ) {
            ...
            if (columnStyleList.size() < k) {
            	columnStyleList.add(wb.createCellStyle());  // 往后面加cellStyle
            }
            copyCell(k - 1, wb, tmpCell, newCell, copyValueFlag);
            k++;
        }
     ...
     */
    private static void copyCell(int index, Workbook wb, Cell srcCell, Cell distCell, boolean copyValueFlag) {
//        CellStyle newStyle = wb.createCellStyle();   // 原来这里没有注释, 现在注释了 意味着单独用就用不来了, 问题也不大 先用着吧
    	
    	CellStyle newStyle = columnStyleList.get(index);
