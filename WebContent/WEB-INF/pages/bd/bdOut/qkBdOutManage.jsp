...
<html lang="zh-CN">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group input-group inline-small">
					    <label >业务员：</label>
					    <input type="text" name="o_id_name" class="form-control search-items" value="${sessionScope.user.userName}">
					    <div class="input-group-btn">
                               <button type="button" class="btn btn-white dropdown-toggle" data-toggle="dropdown">
                                   <span class="caret"></span>
                               </button>
                        ...
                        <input type="hidden" name="o_id" class="form-control search-items" value="${sessionScope.user.job_no}">
					  </div>
	...
	<script type="text/javascript">
		...
		function end_function(index, layero){
			...
			search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getQkBdOutList",$table);
     	}
		...
		//编辑页面
		 function toEdit(f_id,f_department,qk_bd_style,pre_bd_date,next_bd_date,qk_bd_regular,bd_start_date){
			 if(qk_bd_style == 0){
				 ...
				 open_layer_refresh(2,'汇总报出(单期)','qkBdOutPeriodEdit?f_id='+f_id+'&f_department='+f_department,$table,['620px', '580px'],true);
			 }else if (qk_bd_style == 3){
				 open_layer_refresh(2,'汇总报出(变量)','qkBdOutBlEdit?f_id='+f_id+'&f_department='+f_department+'&next_bd_date='+next_bd_date,$table,['620px', '580px'],true);
			 }
		 }	
		...
		function renderOperator(value,row,index){
			var str = '';
			//对时间做处理，否则汇变为毫秒值
			var pre_time_type = new Date(row.pre_bd_date);
