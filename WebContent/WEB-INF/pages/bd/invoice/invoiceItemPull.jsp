...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$('#f_id').val(parent.$('#f_id').val());
		
		$('#f_name').val(parent.$('#f_name').val());

		$(document).ready(function () {
			
			if(parent.$('#is_red').val()==1){
				$("#add").hide();
			}
			...
			$('#commit').on('click', function () {
				var bd_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
	                return {'qk_id':row.qk_id,'period_id':row.period_id};
	            });
				if(bd_ids.length!=0) {
					ajax_function_ask_and_close_json_data("确认引入？","pullInvoiceItem","post",JSON.stringify(bd_ids),'引入成功');
				} else {
					layer.msg("未选择明细！");
				}
			});
			...
			//bootstrap初始化
			initialize_table($table,"",${tableHeader_result},queryParams_result,"#tableTool_result",600);
			...
			$add.on('click', function () {
				var qk_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
	                return {'qk_id':row.qk_id};
	            });
				
				if(qk_ids.length>1){
					layer.msg("选择多条明细，请选择单独的一条做新增");
				}else{
			...
			//button_click_layer_refresh($add,2,"新增期次",'productInstanceAdd?qk_id='+'${qk_id}',$table,false,['1600px', '580px']);
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getInvoicePullList", $table);
			});
