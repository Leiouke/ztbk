...
<!-- 					  </div> -->
					  ...
					  <div class="form-group inline-small">
					    <label >报刊属性：</label>
					    <select name="qk_attribute" class="form-control dictionary search-items" id="qk_attribute">
					    	<option></option>
					    </select>
					  </div>
					  <div class="form-group inline-small">
					    <label >报刊分类：</label>
					    <select name="qk_tye" class="form-control dictionary search-items" id="qk_type">
					    	<option></option>
					    </select>
					  </div>
					  <div class="form-group inline-small">
					    <label >自进标识：</label>
					    <select name="qk_zj_flag" class="form-control dictionary search-items" id="qk_zj_flag">
					    	<option></option>
					    </select>
					  </div>
					  <div class="form-group inline-small">
					    <label >是否停刊：</label>
					    <select name="is_stop" class="form-control search-items" id="is_stop">
					    	<option></option>
					    	<option value = "0">未停刊</option>
					    	<option value = "1">已停刊</option>
					    </select>
					  </div>
				 </div>
	...
	<script type="text/javascript">		
		...
		$(document).ready(function () {
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getProductList",$('#table'));
			});
			//增加条目layer
			var $add=$("#add");
			button_click_layer_refresh($add,2,"新增商品",'productAdd',$table,true,['620px', '580px']);
		 ...
		 function toEdit(qk_id){
			 open_layer_refresh(2,'编辑商品','toProductEdit?qk_id='+qk_id,$table,['620px', '580px'],true);
		 }
		 
		 function toInstance(qk_id){
			 open_layer_refresh(2,'商品刊期','toProductInstance?qk_id='+qk_id,$table,['620px', '580px'],true);
		 }
		 ...
		 function renderOperator(value,row,index){
			 ...
			 if(row.is_stop == 0){
				str = str + "<a onclick=\"toInstance('"+row.qk_id+"')\">刊期</a>";
				str = str + "&nbsp;&nbsp";
				str = str + "<a onclick=\"toEdit('"+row.qk_id+"')\">修改</a>" + "&nbsp;&nbsp"; 
			 }
