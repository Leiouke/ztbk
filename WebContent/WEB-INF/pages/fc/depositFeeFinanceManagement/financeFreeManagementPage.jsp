...
<html lang="zh-CN">
	...
	$(document).ready(function () {	
		...
		$("#search").on('click',function(){
			 ...
			 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getFinanceFreeManagementList",$('#table'));
		 });
		...
		//bootstrap初始化
      	initialize_table($table,"getFinanceFreeManagementList",${tableHeader},queryParams,"#tableTool",400);
		
	});
	
	//加载操作按钮
	function renderOperator(value,row,index){
// 		console.log(row.invoice_no==null);
		var str = "<a onclick=\"toDetail('"+row.fe_id+"')\">详情</a>";
		
// 		str = str+"&nbsp;&nbsp;"
// 	      + "<a onclick=\"toInvoiceSign('"+row.fe_id+"')\">签收</a>";
		
		//有发票号的，未签收的，增加签收按钮
// 		if(!row.is_invoice_sign&&row.invoice_no!=null){
// 			str = str+"&nbsp;&nbsp;"
// 			      + "<a onclick=\"toInvoiceSign('"+row.fe_id+"')\">签收</a>";
// 		}
		//未签收的添加签收按钮
		if(!row.is_invoice_sign&&row.fee_type==1){
			str = str+"&nbsp;&nbsp;"
			      + "<a onclick=\"toInvoiceSign('"+row.fe_id+"')\">签收</a>";
		}
		//没处理的增加处理按钮
		if(!row.is_process){
			str = str +"&nbsp;&nbsp;"
			+"<a onclick=\"toDeal('"+row.fe_id+"')\">处理</a>";
		}
		return str;
	 }
	...
	function toDetail(fe_id){
		open_layer_refresh(2,'详情','toFinanceFreeManagementDetailPage?fe_id='+ fe_id,
			$table,['750px', '500px'],true);
	 }
	
	//处理页面
	function toDeal(fe_id){
		open_layer_refresh(2,'处理','toFreeDealManagementPage?fe_id='+ fe_id,
			$table,['550px', '450px'],false);
	 }
	
	//发票签收
	function toInvoiceSign(fe_id){
		ajax_function_ask_and_refresh("是否确认签收","toInvoiceSign","post",{'fe_id':fe_id},'签收成功',$table);
	 }
