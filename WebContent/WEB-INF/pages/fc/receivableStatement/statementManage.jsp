...
<html lang="zh-CN">
	...
	<script type="text/javascript">		
		...
		load_sum_list_first("${context}/csu/getTableCollectData",{'queryId':'${queryId}', 'rs_type': '${param.rs_type}'});
		$(document).ready(function () {
			...
			button_click_layer_refresh($toImport,2,"引入收款结算单",'statementEdPull',$table,true,['750px', '500px']);  // 目前引的是出口部的结算单, 后续要适配不同部门的
			
			//增加条目layer
			var $add=$("#add");
			var addTitle = "新增收款结算单";
			if (${param.rs_type == 1}) {
				addTitle = "新增其他结算单";
			}
			button_click_windows_end($add, 2, addTitle, 'statementAdd?rs_type=${param.rs_type}', true, ['620px', '500px'], end_function);
			...
			if (${param.rs_type == 1}) {
				//$("#account").hide();
				//$("#return_account").hide();  // 其他结算单 还可以做记账操作的
				$("#toImport").hide();
			}
			...
			$('#account_commit').on('click',function(){
				 if(!has_null($("#account_form"))){
					var rs_ids = $.map($table.bootstrapTable('getSelections'), function (row) {
		                return row.rs_id;
		            });
					var account_month=$("input[name='account_month']").val();
					if(rs_ids.length!=0){
						ajax_function_ask_and_refresh("确认记账？","accountStatement","post",{'rs_ids':rs_ids,'account_month':account_month},'操作成功',$table);
			...
			$('#return_account').on('click',function(){
				 if(true){
					...
					if(rs_ids.length!=0) {
						ajax_function_ask_and_refresh("确认撤销？","deleteStatementAccount","post",{'rs_ids':rs_ids},'操作成功',$table);
				 	} else {
						layer.msg("未选择明细！");
				 	}
				 }
			...
			$('#copy_items').on('click',function(){
				...
				if(rs_ids.length!=0) {
					ajax_function_ask_and_refresh("确认复制?","copyReceivableStatement","post",{'rs_ids': rs_ids},"复制成功!",$('#table'));
				} else {
					layer.msg("未选择明细！");
				}
			});
		 ...
		 function toDel(rs_id) {
			ajax_function_ask_and_refresh("确认删除?","deleteStatement","post",{'rs_id': rs_id},"删除成功!",$('#table'));
		 }
		 
		 function toEdit(rs_id, rs_type){
			open_layer_end(2, '编辑结算单', 'statementEdit?rs_id='+rs_id + '&rs_type=' + rs_type, ['620px', '580px'], true, end_function);
		 }
		 
		 function toKp(rs_id) {
			 open_layer_refresh(2,'快捷开票','statementQuikKp?rs_id='+ rs_id, $table, ['750px', '500px'], true);
		 }
		 
		 function renderOperator(value,row,index) {
			var str="";
			str += "<a onclick=\"toEdit('"+row.rs_id + "','" + row.rs_type + "')\">编辑</a> ";
			// 人工添加的是删除, 引入的是退回
			if (row.source_no == undefined || row.source_no == '') {
				str += "<a onclick=\"toDel('"+row.rs_id + "')\">删除</a> ";
			} else {
				str += "<a onclick=\"toDel('"+row.rs_id + "')\">退回</a> ";
			}
			str += "<a onclick=\"toKp('"+row.rs_id + "')\">快捷开票</a> ";
			return str;
		 }
	</script>
</html>