            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group input-group inline-small">
					    <label >业务员：</label>
					    <input type="text" name="sales_name" class="form-control search-items">
					    <div class="input-group-btn">
                               <button type="button" class="btn btn-white dropdown-toggle" data-toggle="dropdown">
                                   <span class="caret"></span>
                               </button>
					  ...
					  <div class="form-group inline-small">
					    <label >销售创建日期：</label>
					    <input type="text" class="form-control layer-date search-items" name="min_input_time_start" id="min_input_time_start">
					    -
					    <input type="text" class="form-control layer-date search-items" name="min_input_time_end" id="min_input_time_end">
					  </div>
				 </div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getExportInvoiceManageList",$('#table'));
			});
			...
			initialize_bsSuggest_fc_user("${context}", "sales_name", "sales_code");      // 放用户信息
			// 获得当前部门的考核单位
			get_assess_unit_by_dept("${context}", "unit_code");
	    });
		
		function toItem(c_id, export_invoice_no) {
			open_layer_refresh(2,'外销发票收支明细','exportInvoiceItem?c_id='+c_id + '&export_invoice_no=' + export_invoice_no,$table,['720px', '580px'],true);
		}
		 
		function renderOperator(value,row,index) {
			var str="";
		 	str += " <a onclick=\"toItem('"+row.custom_f_id + "','" + row.export_invoice_no + "')\">收支明细</a>";
			return str;
		}
	</script>
</html>