...
<!-- 								    <input type="text" name="is_red" id = "is_red" class="form-control search-items" readonly> -->
								  </div>
							</div>
							<div class="row">
								 <div class="form-group input-group inline-small not-null">
								  ...
								  <div class="form-group inline-small not-null">
								    <label >发票外币总金额：</label>
								    <input type="text" name="c_real_money" id="c_real_money" class="form-control search-items">
								  </div>
								  <div class="form-group inline-small">
								    <label >累加金额：</label>
								    <input type="text" name="sum_money" id="sum_money" class="form-control search-items" disabled="disabled">
								  </div>
							</div>
							<div class="row">
								  <div class="form-group input-group inline-large">
								    <label >备注：</label>
								    <input type="text" name="memo" id = "memo" class="form-control search-items">
								  </div>
							      <div class="form-group inline-small not-null">
			                  		<label>所属组织机构：</label>
				                  ...
				                  <div class="form-group inline-small not-null">
					  			  ...
					  			  <!-- <div class="form-group inline-small">
									<label>发票文件：</label>
								    <input type="file" name="file_address" id="file_address" class="form-control" style="height:30px;">
								  </div> -->			
							</div>
						 </div>
						</form>
	...
	$(document).ready(function () {	
		...
		$('#save').on('click', function () {
			if(!has_null($("#form"))&&!auto_not_matching($("#form"))){				
				if($("input[name='c_id']").attr("data-id")=="") {
					layer.msg("客户编号不正确");
				}else if('${is_red}'==1&&$("#c_real_money").val()>=0){
					layer.msg("红冲发票金额必须为负数");
				}else if('${is_red}'==0&&$("#c_real_money").val()<0){
					layer.msg("非红冲发票金额必须大于等于0");
				}else{
					$("#is_red").removeAttr("disabled");
					...
					if(confirm("是否新增?")){
						...
						ajax_function("addSourceInvoice","post",$('#form').serialize(),'新增成功');
					}
				}
			}
		});
		...
		//bootstrap初始化		
		initialize_table($table,"getSourceInvoiceAddItemList",${tableHeader},queryParams,"#tableTool",550);
 		$table.bootstrapTable('refreshOptions',{onLoadSuccess: function(data){
 			...
 			$("input[name='sum_money']").val(sum_money);
		}});
		
		//增加条目layer
		var $add=$("#add");
		
		//button_click_windows_end($add,2,"引入结算单","sourceInvoiceStatementItemPull?f_id="+$('#f_id').val()+"&f_department="+$('#f_department').val(),true,['750px', '580px'],end_function);
		
		$add.on('click', function () {
		  var f_id = $("#f_id").val();
		  //var f_department = $("#f_department").val();
		  var url = "sourceInvoiceStatementItemPull?f_id="+f_id
