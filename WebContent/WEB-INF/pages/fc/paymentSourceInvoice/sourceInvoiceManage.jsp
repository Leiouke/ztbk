         ...
         <div class="ibox float-e-margins">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group inline-small">
					    <label >发票状态：</label>
					    <select name="fc_invoice_status" class="form-control dictionary search-items" >
					    	<option></option>
					    </select>
					  </div>					  
...
<!-- 					  </div> -->
	...
	<script type="text/javascript">
		...
		function end_function(index, layero){
			ajax_function_no_close("returnSourceInvoice","post",{},null);
			var json_data_list=formToJson($("#form_search"));
			json_data_list['queryId']='${queryId}';
			search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getSourceInvoiceManageList",$('#table'));
     	}
...
// 		var kp_layer_index;
		$(document).ready(function () {
			...
			//增加条目layer
			var $add=$("#add");
			button_click_windows_end($add,2,"新增蓝票",'sourceInvoiceAdd?is_red=0',true,['620px', '500px'],end_function);
			//增加条目layer
			
			var $red_add=$("#red_add");
			button_click_windows_end($red_add,2,"新增红票",'sourceInvoiceAdd?is_red=1',true,['620px', '500px'],end_function);
		 ...
		 function toEdit(inv_id){
			 open_layer_end(2,'编辑开票','sourceInvoiceEdit?inv_id='+inv_id+'&is_detail='+0,['620px', '580px'],true,end_function);
		 }
		 
		 function toDetail(inv_id){
			 open_layer_end(2,'开票详情','sourceInvoiceEdit?inv_id='+inv_id+'&is_detail='+1,['620px', '580px'],true,end_function);
		 }
		 
		 function toDel(inv_id){
			 ajax_function_ask_and_refresh("确认删除","deleteSourceInvoice","post",{'inv_id':inv_id},'操作成功',$table);
		 }
		 
		 function toUpload(inv_id){
			 /*$("#toPic").click(function(){
					var ap_id = $("#ap_id").val();
					var detail = '${is_detail}';
					if('${is_detail}'==1){
						open_layer_end(2,'附件详情','toCashApplicationPicEdit?ap_id='+ap_id+'&detail='+detail,['1200px', '600px'],true,end_function)
					}else{
						open_layer_end(2,'修改附件','toCashApplicationPicEdit?ap_id='+ap_id+'&detail='+detail,['1200px', '600px'],true,end_function)
					}
				});*/
			 open_layer_refresh(2,'上传发票文件','toPaymentInvoicePicEdit?order_no='+inv_id,$table,['1200px', '600px'],false);
		 }
		 
		 function renderOperator(value,row,index){
			var str = '';
			if(row.status!=0||row.is_against==1){ //不为制单中或者是已冲销
			...
			/*if (row.is_upload_file) {
				str += "&nbsp;&nbsp;<a href='downloadPaymentInvoiceFile?filename=" + row.file_address + "')\">下载</a>";
			}*/
			
			str += "&nbsp;&nbsp;<a onclick=\"toUpload('" + row.inv_id + "')\">附件管理</a> ";
			return str;
		 }
	</script>
</html>