...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			if (${param.ps_type == 0}) {
				// 获得当前部门的财务分类(取普通型)
				get_financial_classify_by_dept("${context}", "${sessionScope.defaultRole.dep_org_code}", false, "ps_cw_type");
			} else if (${param.ps_type == 1}) {
				// 获得当前部门的财务分类(取其他型)
				get_financial_classify_by_dept("${context}", "${sessionScope.defaultRole.dep_org_code}", true, "ps_cw_type");
			} else if (${param.ps_type == 2}) {
				// 获得费用的财务分类
				get_financial_classify_by_dept("${context}", "0000", false, "ps_cw_type");
			}
