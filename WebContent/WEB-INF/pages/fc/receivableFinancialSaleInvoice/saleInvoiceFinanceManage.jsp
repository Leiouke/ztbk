...
<html lang="zh-CN">
    ...
    <div class="sonpage-footer">
	    ...
	    <button id="export_wb_kp_finance" class="btn btn-success" type="button">导出外部开票明细</button>
	</div>
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $('#export_kp_finance').on('click', function () {
				 ...
				 if(kp_ids.length!=0){
					...
					var href_str="${context}/fc/download/kpInvoiceFinanceExport?";
			 ...
			 $('#export_wb_kp_finance').on('click', function () {
				 var has_common = 0;
				 var has_zengzhi = 0;
				 var kp_ids = $.map($table.bootstrapTable('getSelections'), function (row) {
					if (row.fc_kp_form == '0'){
				 ...
				 if(kp_ids.length!=0){
					//alert("导出完成后请重新查询查看对应的导出批次号！");
					var href_str="${context}/fc/download/wbkpInvoiceFinanceExport?";
					href_str+="kp_ids="+kp_ids;
					window.open( href_str);
				 }else {
					layer.msg("未选择明细！");
