...
<!-- 								  </div> -->
								  ...
								  <div class="form-group inline-large input-group">
<!-- 								    <label >fb_id</label> -->
								    <input type="text" name="fb_id" id="fb_id" class="form-control search-items" style="display:none">
								  </div>
							</div>
						 </div>
						</form>
	...
	<div class="sonpage-footer">
	    <button id="edit" class="btn btn-primary" type="button">修改</button>
	...
	$(document).ready(function () {
		...
		if("${param.to_detail}"==1){
			$("#add").hide();
			$("#edit").hide();
		}
		
// 		console.log("${param.to_detail}");
		...
		$('#edit').on('click', function () {
			if(!has_null($("#form"))){
				if($("input[name='f_id']").attr("data-id")=="") {
					...
					ajax_function("editFactoryPayItem","post",$('#form').serialize(),'更新成功');
				}
			}
		});
