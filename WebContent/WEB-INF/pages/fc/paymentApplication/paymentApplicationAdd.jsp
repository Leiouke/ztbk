	...
	$(document).ready(function () {
		...
		$('#save').on('click', function () {
			if(!has_null($("#form"))){
				...
				ajax_function("addPaymentApplication","post",$('#form').serialize(),'新增成功');
			}
		});
		...
		//加载字典数据,为预付单则为付款类型定死为预付，其他随意
		if('${type}'==0){
			var myObject = {
					oa_payment_type: '2'
					};
			var jsonString = JSON.stringify(myObject);
			var josn_string=JSON.parse(jsonString);
			get_dictionary("${context}",josn_string);
		}else{
		...
		//bootstrap初始化
		if('${type}'==0){
			initialize_table($table,"getPaymentApplicationItemList",${tableHeader},queryParams,"#tableTool",400);
		}else{
			initialize_table($table,"getPaymentApplicationItemList",${invoicetableHeader},queryParams,"#tableTool",400);
		}
