			            ...
			            <div class="col-xs-12">
							...
							<div class="row">
						        <div class="form-group large-space col-xs-12" >
						            <label>汇款附言：</label>
				                    <textarea  name="remittance_information" rows="10" ></textarea>
						        </div>
							</div>
						 </div>
						</form>
	...
	$(document).ready(function () {
			$('#save').on('click', function () {
				if(!has_null($("#form")) && !auto_not_matching($("#form"))){
					ajax_function("createApplicationPaymentNotice","post",$('#form').serialize(),'保存成功');
				}
			});
			...
			$('#saveExport').on('click', function () {
				if(!has_null($("#form")) && !auto_not_matching($("#form"))){
					ajax_function_dosuccess("createApplicationPaymentNotice","post",$('#form').serialize(),function(){
						 var href_str="${context}/fc/print/paymentApplicationNotice?is_print=0&";
						 var ap_id=$("#form").find("input[name='ap_id']").val();
			...
			$('#savePrint').on('click', function () {
				if(!has_null($("#form")) && !auto_not_matching($("#form"))){
					ajax_function_dosuccess("createApplicationPaymentNotice","post",$('#form').serialize(),function(){
						...
						ajax_function_no_close("${context}/fc/print/paymentApplicationNotice","post",$('#form').serialize(),"发送打印完成！"); 
					});
				}
			});	
