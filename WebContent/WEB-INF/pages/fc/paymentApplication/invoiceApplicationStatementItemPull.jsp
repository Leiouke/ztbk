	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$('#commit').on('click', function () {
				var send_data= $.map($table.bootstrapTable('getSelections'), function (row) {
	                return {'bill_id':row.inv_id, 'c_real_money':row.can_apply_c_real_money, 'real_money':row.can_apply_real_money};
	            });
				if(send_data.length!=0) {
					ajax_function_ask_and_close_json_data("确认引入？","pullPaymentInvoiceItem","post",JSON.stringify(send_data),'引入成功');
				} else {
					layer.msg("未选择明细！");
				}
			});
			...
			$("#search_pull_items").on('click', function () {
				...
				search_sum_list("${context}/csu/getTableCollectData", json_data_list, "getInvoiceStatementItemPullList", $table);
			});
