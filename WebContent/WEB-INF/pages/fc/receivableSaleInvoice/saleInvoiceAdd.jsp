...
<html lang="zh-CN">
			            ...
			            <div class="col-xs-12">
							...
							<div class="row">
								  ...
								  <div class="form-group inline-small not-null">
								    <label >开票类别：</label>
								    <!--<select name="fc_kp_form" id = "fc_kp_form" data-id-start=3 data-id-end=4 class="form-control dictionary search-items" >-->
							...
							<div class="row">
							      ...
							      <div class="form-group inline-small not-null">
			                        ...
			                        <input type="hidden" name="fc_kp_type" value="${param.fc_kp_type}"/>  <!-- 发票类型 -->
				                  </div>
	...
	function renderOperator(value,row,index){
	 	//console.log(row);
		return "<a onclick=\"toDel('" + row.statement_id + "')\">删除</a>";
	}
	...
	$(document).ready(function () {
		...
		$('#save').on('click', function () {
			...
			if(!has_null($("#form"))&&!auto_not_matching($("#form"))){				
				if($("input[name='c_id']").attr("data-id")=="") {
					...
					ajax_function("addSaleInvoice","post",$('#form').serialize(),'新增成功');
				}
			}
		});
		...
		// 获得当前部门的财务分类(取普通型)
		get_financial_classify_by_dept("${context}", "${sessionScope.defaultRole.dep_org_code}", false, "kp_cw_type");
