...
<html lang="zh-CN">
			            ...
			            <form id="form" class="form-inline">
							<div class="col-xs-12">
								<div class="row">
									...
									<div class="form-group input-group inline-small not-null">
										<label >发票号：</label>
									    <input type="text" name="inv_no" class="search-items" id = "inv_no"/>
	                                 </div>
									<div class="form-group input-group inline-small not-null">
										<label >合计金额：</label>
									    <input type="text" name="total_real_money" class="search-items"  id = "total_real_money"/>
	                                 </div>
	                                 <div class="form-group input-group inline-small not-null">
										<label >合计税额：</label>
									    <input type="text" name="total_tax_money" class="search-items" id = "total_tax_money"/>
	                                 </div>
	                       		</div>
	                       		<div class="row">
	                        		<div class="form-group input-group inline-small not-null">
	...
	$(document).ready(function () {
		...
		$('#save').on('click', function () {
			$('#dcl_id').removeAttr("disabled");
			$('#detail_real_money').removeAttr("disabled");
			$('#detail_tax_money').removeAttr("disabled");
			$('#detail_pre_taxback').removeAttr("disabled");
			if(!has_null($("#form"))){
				ajax_function("editDecalreInvoice","post",$('#form').serialize(),'保存成功');
			}
		});
		...
		$('#delete_items').on('click',function(){
			...
			if(inc_nos.length!=0) {
				ajax_function_ask_and_refresh("确认删除?","deleteInvoiceTaxbackItemTemp","post",{'inc_nos': inc_nos},"删除成功!",$('#table'));
			} else {
				layer.msg("未选择明细！");
			}
		});
		...
		//bootstrap初始化
		initialize_table($table,"getTaxbackInvoiceItemTempList",${tableHeader},queryParams,"#tableTool",400);
		...
		//增加条目layer
		$("#add").click(function(){
			open_layer_refresh(2,'新增报关发票明细','taxbackInvoiceItemTempAdd?dcl_id='+'${dcl_id}',
					$table,['800px', '300px'],false);
		});
		...
		$table.bootstrapTable('refreshOptions',{onLoadSuccess: function(data){
			$.ajax({
		        url: "getDeclareInvoiceItemSum?guid="+'${taxback_invoice_guid}', 
		        method: "post",
		        dataType: "json",
		        traditional:true,
		        success: function (data) {
		        	console.log("data="+data);
		        	$('#detail_real_money').val(data.real_money);
		        	$('#detail_tax_money').val(data.tax_money);
		        	$('#detail_pre_taxback').val(data.pre_taxback);
		        },
		        error:function(data){	
		        	alert("请求失败!");	
		        }
	...
	function toDel(inc_no) {
		ajax_function_ask_and_refresh("确认删除?","deleteInvoiceTaxbackItemTemp","post",{'inc_nos': inc_no},"删除成功!",$('#table'));
	}
