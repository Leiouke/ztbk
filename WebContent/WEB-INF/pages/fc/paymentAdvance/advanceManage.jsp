...
<html lang="zh-CN">
	...
	<script type="text/javascript">		
		...
		$(document).ready(function () {
			...
			initialize_table_editable($table,"getAdvanceList",${tableHeader},queryParams,"#tableTool",500,"editPaymentAdvanceExportInvoiceNo");
			
			//增加条目layer
			$("#add").click(function(){
				open_layer_refresh(2,'新增预付单','advanceAdd',
						$table,['850px', '450px'],false);
			});
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getAdvanceList",$('#table'));
			});
		 ...
		 function toDel(pv_id) {
			ajax_function_ask_and_refresh("确认删除?","deletePaymentAdvance","post",{'pv_id': pv_id},"删除成功!",$('#table'));
		 }
		 
		 function toEdit(pv_id){
			 open_layer_refresh(2,'编辑预付单','advancePrymentEdit?pv_id='+pv_id+'&is_detail='+0,$table,['850px', '450px'],false);
		 }
		 
		 function toDetail(pv_id){
		 ...
		 function renderOperator(value,row,index) {
			var str="";
			if(row.status==0){
				str += "<a onclick=\"toEdit('"+row.pv_id+"')\">编辑</a> ";
				str += "<a onclick=\"toDel('"+row.pv_id + "')\">删除</a> ";
			}else{
				str += "<a onclick=\"toDetail('"+row.pv_id+"')\">详情</a> ";
			}
			return str;
		 }
	</script>
