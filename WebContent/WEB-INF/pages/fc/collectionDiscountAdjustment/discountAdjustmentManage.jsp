...
<html lang="zh-CN">
	...
	<script type="text/javascript">		
		...
		$(document).ready(function () {
			...
			initialize_table($table,"getDiscountAdjustmentList",${tableHeader},queryParams,"#tableTool",500);
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getDiscountAdjustmentList",$('#table'));
			});
			...
			$('#account_commit').on('click',function(){
				 if(!has_null($("#account_form"))){
					...
					if(bill_ids.length!=0){
						ajax_function_ask_and_refresh("确认记账？","accountDiscountAdjustment","post",{'bill_ids':bill_ids,'account_month':account_month},'操作成功',$table);
			...
			$('#return_account').on('click',function(){
				 if(true){
					...
					if(bill_ids.length!=0) {
						ajax_function_ask_and_refresh("确认撤销？","deleteDiscountAdjustmentAccount","post",{'bill_ids':bill_ids},'操作成功',$table);
				 	} else {
						layer.msg("未选择明细！");
				 	}
				 }
		 ...
		 function toItem(bill_id) {
			open_layer_refresh(2,'折扣明细','discountAdjustmentItems?bill_id=' + bill_id,$table,['750px', '500px'],true);
		 }
