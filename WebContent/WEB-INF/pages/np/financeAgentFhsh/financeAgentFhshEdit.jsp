...
<html lang="zh-CN">
							...
							<div class="row">
								<div class="form-group input-group inline-middle not-null">
								  ...
								  <div class="form-group inline-small">
								    <label >账户余额：</label>
								    <input type="text" name="advance_balance" class="form-control search-items" value="0" readonly>
								  </div>
							</div>
							<div class="row">
								<div class="form-group inline-small">
	...
	load_sum_list_first("${context}/csu/getTableCollectData",{'queryId':'${queryId}','guidname':'finance_agent_fhsh_guid'});
	var $table = $('#table');
		$(document).ready(function () {
			...
			$('#save').on('click', function () {
				if(!has_null($("#form"))&&!auto_not_matching($("#form"))){
					if($("input[name='f_id']").attr("data-id")==""){
						...
						ajax_function_no_close("editAgentFinanceFhsh","post",$('#form').serialize(),'保存成功');
					}
				}
			 });
			...
			$('#commit').on('click', function () {
				if(!has_null($("#form"))){
					if($("input[name='f_id']").attr("data-id")==""){
						...
						ajax_function("commitAgentFinanceFhsh","post",$('#form').serialize(),'提交成功');					
					}
				}
			 });
			...
			$('#delete').on('click',function(){
				 ajax_function_ask_and_close("确认删除？","deleteFinanceAgentFhsh","post",{fh_id:finance_fhsh.fh_id},'删除成功');
			});
			//bootstrap初始化
			initialize_table_editable_footer($table,"getFinanceAgentFhshItemList",${tableHeader},queryParams,"#tableTool",550,"");
			...
			//增加条目layer
			var $add=$("#add");
			button_click_layer_refresh($add,2,"引入集货单明细",'financeAgentFhshItemPull',$table,true,['750px', '580px']);
			...
			$('#delete_items').on('click',function(){
				...
				if(inv_ids.length!=0)
					ajax_function_ask_and_refresh("确认删除？","deleteAgentFinanceFhshItem","post",{'inv_ids':inv_ids},'删除成功',$('#table'));
				else
					layer.msg("未选择明细！");
			});
			//汇总详情
