...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			//bootstrap初始化
			initialize_table($table,"getCostAccountingManageList",${tableHeader},queryParams,"#tableTool",400);
			$("#search").on('click',function(){
				 var json_data_list=formToJson($("#form_search"));
				 json_data_list['queryId']='${queryId}';
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getCostAccountingManageList",$table);
			});
		...
		$("#batchImport").on('click',function() {
			ajax_function_ask_and_refresh("确认从益华同步数据?","syncCostAccounting","post",{},"同步成功!",$('#table'));
		});
		
		function toEdit(inc_id) {
			open_layer_refresh(2,'成本修改','toCostAccountingEdit?inc_id='+inc_id,$table,['750px', '500px'],false);
		}
		
		function renderOperator(value,row,index) {
			var str="";
			str += "<a onclick=\"toEdit('"+row.inc_id + "')\">修改</a>";
			return str;
	    }
	</script>
</html>