...
<html lang="zh-CN">
	...
	<div class="row wrapper border-bottom white-bg page-heading">
	        <div class="col-sm-4">
	            ...
	            <ol class="breadcrumb">
	                <li>
	                    <a id="" href="statementAgencyItemPull">代销业务</a>
	                </li>
	                <li>
	                    <a id="" href="statementCostItemPull">费用业务</a>
	                </li>
	                <li>
	                    <strong>收订业务</strong>
	                </li>
        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">	
					  ...
					  <div class="form-group inline-middle" >
				            <label >推广类型：</label>
				            <input type="text" name="c_level_name" class="form-control search-items">
						    <div class="input-group-btn">
	                         <button type="button" class="btn btn-white dropdown-toggle" data-toggle="dropdown">
	                             <span class="caret"></span>
	                         </button>
	                        ...
	                        <input type="hidden" name="c_level" class="form-control search-items">
				      </div> 
				      <div class="form-group input-group inline-small">
					    <label>录入人：</label>
	...
	<script type="text/javascript">		
		...
		load_sum_list_first("${context}/csu/getTableCollectDataBK",{'queryId':'${queryId}'});
		$(document).ready(function () {			
			...
			$("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectDataBK",json_data_list,"getStatementSubscriptionItemPullList",$('#table'));
			});
		...
		$("#batchImport").on('click',function() {
			...
			var item_ids= $.map($table.bootstrapTable('getSelections'), function (row) {
                return {'custom_s_id':row.c_id,'source_no':row.zd_id,'sourece_type':'03','real_money':row.real_money,'total_money':row.total_money,'memo':row.memo};
            });
			
			if(item_ids.length != 0) {
				ajax_function_ask_and_close_json_data("确认引入?","importStatementItem","post",JSON.stringify(item_ids),"引入完成!");
				
				// 点了之后 按钮让他灰一段时间, 设定3秒, 保证不会连续提交
				$("#batchImport").attr("disabled", true);
				setTimeout(function() {
					$("#batchImport").attr("disabled", false);
				}, 3000);
			} else {
				layer.msg("未选择明细！");
			}
		...
		function toItem(zd_id) {
			open_layer_refresh(2,'分类明细-收订业务','statementSubscriptionItems?zd_id='+zd_id,$table,['750px', '500px'],true);
		}
		
		function renderOperator(value,row,index) {
			var str="";
			str += "<a onclick=\"toItem('"+row.zd_id+"')\">分类明细</a>";
			return str;
		}
	</script>
</html>