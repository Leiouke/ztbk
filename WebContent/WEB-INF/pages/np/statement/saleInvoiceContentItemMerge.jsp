...
<html lang="zh-CN">
	...
	<body>
			   ...
			   <div class="col-md-12">
				    ...
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
	                  		<label>税收分类：</label>
	                        <select id="tax_class_code" name="tax_class_code" class="dictionary search-items"></select>
		                </div>  
				    </div>
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
	                  		<label>数量：</label>
	                        <input type="text" name="kp_amount" value="${param.kp_amount}">
		                </div>  
				    </div>
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
				    ...
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8 not-null">	
	                  		<label>金额：</label>
	                        <input type="text" name="kp_money" value="${param.kp_money}">
		                </div>  
				    </div>
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
				    ...
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
	                  		<label>计算折扣：</label>
	                        <input type="text" name="discount_online" disabled>
		                </div>  
				    </div>
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
				    ...
				    <div class="row">
				    	<div class="form-group middle-space col-xs-8">	
	                  		<label>码洋：</label>
	                        <input type="text" name="total_money" value="${param.total_money}">
		                </div>  
				    </div>
				    <div class="row">						    	
		                <div class="form-group middle-space col-xs-8">	
		...
		<div class="sonpage-footer">
	        <button id="save" class="btn btn-primary" type="button">合并</button>
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			// 填进去计算折扣
			$("input[name='discount_online']").val(('${param.total_money}' - '${param.kp_money}')/'${param.total_money}');
			...
			$('#save').on('click', function () {
				...
				var tax_class_code = $("#tax_class_code").val();
				
				if (tax_class_code != '10602010199'
					  && (kp_amount != '' || kp_unit != '')
					) {  // 不是印刷品分类, 开票数量 和 开票单位 不能有内容
					alert("非印刷品分类不能有数量和单位内容！");
					return;
				}
				
				if(!has_null($("#form"))){
					ajax_function_ask_and_close("确认合并发票内容明细？","mergeSaleInvoiceContentItem","post",$("#form").serialize(),"合并完成！");
				}
			 });
		});
	</script>
