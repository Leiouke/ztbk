...
<html lang="zh-CN">
	    ...
	    <div class="ibox float-e-margins">
            ...
            <div class="ibox-content search-content">
	            ...
	            <div class="col-xs-11">
					  ...
					  <div class="form-group inline-small">
					    <label >客户名称(模糊)：</label>
					    <input type="text" name="custom_name" class="form-control search-items" >
					  </div>				  
				 </div>
        ...
        <div class="ibox float-e-margins" style="margin-bottom:0px">
            ...
            <div class="ibox-content ibox-content-table">
                ...
                <div class="row row-lg" style="margin-top: 5px;text-align: center;">
              		<button type="button" class="btn btn-primary" id="select_in">认领选中的发票</button>
                </div>
            </div>
        </div>
   </div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			//bootstrap初始化
			initialize_table($table_result,"getPayableSaleInvoiceToCashClaimList",${tableHeader},queryParams_result,"#tableTool_result",350);
			$("#select_in").on('click', function () {			
				var item_ids= $.map($table_result.bootstrapTable('getSelections'), function (row) {
	                return {'rp_id':'${param.rp_id}','c_id':row.c_id,'money_amount':row.can_receive_money,'kp_id':row.kp_id};
	            });
				
				if(item_ids.length != 0) {
					ajax_function_ask_and_close_json_data("确认认领?","importCashSaleInvoice","post",JSON.stringify(item_ids),"认领完成!");
				} else {
					layer.msg("没有选中的发票记录！");
				}
			});
			...
			$("#search").on('click',function(){
				...
				search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getPayableSaleInvoiceToCashClaimList",$table_result);
			});
	     });
	</script>
</html>