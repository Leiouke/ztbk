...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		function queryParams(param) {
			...
			json_obj_cash_register['rp_id']=cash_register.rp_id;
		    return json_obj_cash_register;
		}
		
		var $table = $('#table');
		
		load_sum_list_first("${context}/csu/getTableCollectData",{'queryId':'${queryId}', 'rp_id': cash_register.rp_id});
		$(document).ready(function () {
			...
			//bootstrap初始化
			initialize_table($table,"getcashClaimInvoiceManageList",${tableHeader},queryParams,"#tableTool",400);
			//表格配置 layer
			var $config=$("#config");
			...
			var $add=$("#add");
			$add.click(function(){
				open_layer_refresh(2,"引入发票","payableSaleInvoiceToCashClaim?rp_id=" + cash_register.rp_id,$table,['750px', '580px'],true);
			});
		});	
			
		function toDel(rp_id, kp_id, income_id, pay_id) {
			 ajax_function_ask_and_refresh("是否撤销认领？",'deleteCashSaleInvoice',"post",{'rp_id':rp_id, 'kp_id':kp_id, 'income_id':income_id, 'pay_id':pay_id},"撤销成功",$('#table'));
		}
			
		function renderOperator(value,row,index){
			 var str = "<a onclick=\"toDel('" + row.rp_id + "','" + row.kp_id + "','" + row.income_id + "','" + row.pay_id + "')\">撤销</a>";
			 return str;
		}
	</script>
</html>