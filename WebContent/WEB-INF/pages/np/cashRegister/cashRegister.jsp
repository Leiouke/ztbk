...
<html lang="zh-CN">
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			//增加条目layer
			var $add=$("#add");
			button_click_layer_refresh($add,2,"新增到款",'cashRegisterAdd',$table,false,['750px', '400px']);
			var $upload=$("#upload");
			button_click_layer_refresh($upload,2,"导入到款",'cashRegisterUpload',$table,false,['750px', '400px']);
			//表格配置 layer
			var $config=$("#config");
			...
			$("#search").on('click',function(){
					...
					search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getCashRegisterList",$table);
			});
			//自动搜索填补
			initialize_bsSuggest_client("${context}");
	     });
		 
		 function toEdit(rp_id){
			open_layer_refresh(2,'编辑到款','cashRegisterEdit?rp_id='+rp_id,$table,['750px', '400px'],false);
		 }
		 
		 function toDel(rp_id){
			ajax_function_ask_and_refresh("是否删除","deleteCashRegister","post",{'rp_ids':rp_id},"删除成功",$('#table'));
		 }
		 
		 function renderOperator(value,row,index){
			<c:if test="${param.isEditable == null or param.isEditable}">
		 	var str="<a onclick=\"toEdit('"+row.rp_id+"')\">编辑</a>";
			if(true){
				str+="&nbsp;&nbsp;<a onclick=\"toDel('"+row.rp_id+"')\">删除</a>";
			}
			return str;
			</c:if>
		 }
