...
<html lang="zh-CN">
	...
	function queryParams(param) {
		var json_obj_outbound = {};
		json_obj_outbound['limit'] =param.limit;
		json_obj_outbound['offset'] =param.offset;
		json_obj_outbound['sortName'] =this.sortName;
		json_obj_outbound['sortOrder'] =this.sortOrder;
	    return json_obj_outbound;
	}
	
	function toDel(id,bd_id,inv_id,item_code){
		ajax_function_ask_and_dosuccess("是否删除","deleteBackIssueOutBoundItemVerify","post",{'ids':id},load_table_data_success);
	}
	...
	$(document).ready(function () {
			$('#memo_commit').on('click', function () {
				...
				if(ids.length!=0)
					ajax_function_ask_and_close("确认出库验货完成？","addBackIssueOutBound","post",$('#form').serialize(),'过刊出库单完成！');
				else
					layer.msg("本次没有验货记录！");
			});
			
			$('#commit').on('click', function () {
				memo_layer_index=open_layer(1,"出库单完成",$('#memo_div'),['360px', '280px'],false);
			});
				        ...
				        success: function (data) { 
				        	if (data.success){
				        		...
				        		if(result==0){
				        			...
				        			open_layer_end(2,'选择对应期刊明细验入','backIssueOutBoundConfirm',['620px', '580px'],true,end_function);
				        		}		        		
				        	} else {
				        		alert(data.errorMsg);
				        	}
			...
			function verify_success_function(data) {
				...
				if(!has_null($("#form"))){
					ajax_function_refresh("confirmBackIssueOutBound","post",$('#form').serialize(),'验货成功',$table);
				}
			}
			...
			//bootstrap初始化
			initialize_table($table,"getBackIssueOutBoundAddList",${tableHeader},queryParams,"#tableTool",550);
			//表格配置 layer
			var $config=$("#config");
