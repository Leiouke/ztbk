...
<html lang="zh-CN">
				...
				<div class="col-xs-12">
					<div class="row">
					 	<div class="form-group inline-small not-null">
							 <label >下架单号：</label>
							 <input name = "source_orderno" id = "source_orderno" class="form-control search-items" readonly>
						</div>
					</div>
				</div>
			</form>
	...
	function load_table_data_success(data){
		if (data.success){
   			layer.msg("操作成功！");
   			load_sum_list("${context}/csu/getTableCollectData",{'queryId':'${queryId}','guidname':'sendOut_guid'},$table);
    	} else {
	...
	var $table = $('#table');
	load_sum_list_first("${context}/csu/getTableCollectData",{'queryId':'${queryId}','guidname':'sendOut_guid'});
	$(document).ready(function () {
		
		//增加条目layer
		var $add=$("#add");
		button_click_layer_refresh($add,2,"引入下架单",'sendOutItemTempAdd',$table,true,['620px', '580px']);
		
		$('#save').on('click', function () {
			
			if(!has_null($("#form"))){
					ajax_function_ask_and_close("确认新增？","addSendOut","post",$('#form').serialize(),'新增成功');
			}
		...
		//bootstrap初始化		
		initialize_table_editable($table,"getSendOutItemTempList",${tableHeader},queryParams,"#tableTool",550,"editSendOutItemTemp");
