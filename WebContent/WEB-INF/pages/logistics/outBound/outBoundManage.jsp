	...
	<script type="text/javascript">
		...
		function queryParams(param) {
		 ...
		 $(document).ready(function () {
			...
			table_column_config_layer($config,2,'表格属性配置','${context}/csu/columnConfig?queryId=${queryId}');
			 ...
			 $("#search").on('click',function(){
				 ...
				 search_sum_list("${context}/csu/getTableCollectData",json_data_list,"getOutBoundList",$table);
			 });
			 //自动搜索填补
		 ...
		 function toEdit(downshelf_id){
			 open_layer_refresh(2,'编辑出库明细','outBoundEdit?downshelf_id='+downshelf_id,$table,['360px', '380px'],true);
		 }
		 
		function toDownshelf(downshelf_id){
			open_layer_refresh(2,'出库单下架','outBoundShelvesConfirm?downshelf_id='+downshelf_id,$table,['360px', '380px'],true);
	 	}
		
		 function renderOperator(value,row,index){
			 var str = "";
			 if(row.is_downshelves==1){//已经下架的只能看详情
				 str="&nbsp;&nbsp;<a onclick=\"toEdit('"+row.downshelf_id+"')\">详情</a>";
// 				 str+="&nbsp;&nbsp;<a onclick=\"toDownshelf('"+row.downshelf_id+"')\">下架</a>";
			 }else if(row.out_type=="01"){//未下架的配送出库单，不让仓库编辑和删除，只能下架和看详情
				 str+="<a onclick=\"toEdit('"+row.downshelf_id+"')\">详情</a>";
				 str+="&nbsp;&nbsp;<a onclick=\"toDownshelf('"+row.downshelf_id+"')\">下架</a>";
			 }else{//剩余未下架的单据，给与编辑，下架和删除
