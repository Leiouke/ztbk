...
<html lang="zh-CN">
				            ...
				            <div class="col-xs-12">
					            <div class="row">
					            	   <div class="form-group inline-small not-null">
									    <label >出库类型：</label>
									    <select name="out_type" id = "out_type" data-id-start ="02" data-id-end ="05" class="form-control dictionary search-items">
									    </select>
									  </div>
								</div>
								<div>
	...
	$(document).ready(function () {
		...
		if(outBound_item.is_downshelves==1||outBound_item.out_type=="01"){
			$('#save').hide();
			$('#delete_items').hide();
			$('#add').hide();
		...
		var downshelf_id = outBound_item.downshelf_id;
		
		button_click_layer_refresh($add,2,"新增出库单明细",'outBoundItemTempAdd?downshelf_id='+downshelf_id,$table,true,['620px', '580px']);
		
		$('#save').on('click', function () {
			
			if(!has_null($("#form"))){
					ajax_function_ask_and_close("确认修改？","editOutBound","post",$('#form').serialize(),'修改成功');
			}
		...
		//bootstrap初始化		
		initialize_table($table,"getOutBoundItemTempList",${tableHeader},queryParams,"#tableTool",550);
